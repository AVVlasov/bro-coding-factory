...
It 'гард verify берёт журнал из того же каталога состояния, что и шина' {
    $v = Get-Content -Raw (Join-Path $root 'harness\verify.ps1')
    # Шаблон в ОДИНАРНЫХ кавычках. В двойных PowerShell подставлял сюда настоящий
    # $PSScriptRoot: в пути вроде ...\M-02 получалось «\M» — недопустимая escape-
    # последовательность, и тест падал разбором регекспа вместо проверки файла. В путях
    # без такой пары (\b, \d) он молча проверял не то, что написано.
