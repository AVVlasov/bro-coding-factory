...
# --- Гейт объёма видит прошлые циклы ветки ------------------------------------------------
#
# 2026-09-10, TASK-07: база «дерево на старте цикла» не видела package.json пакета и сквозной
# тест, добавленные прошлым прогоном той же ветки; гейт пропустил, поймал судья яруса.
# Собственные коммиты ветки опознаются по трейлеру Task:, файлы из них входят в проверку.

It 'гейт объёма учитывает файлы прошлых коммитов ветки с трейлером Task:' {
    $v = Get-Content -Raw (Join-Path $root 'harness\verify.ps1')
    $m = [regex]::Match($v, '(?s)\$changedAll = @\(\).*?\$changedAll = @\(\$changedAll \| ForEach-Object')
    Assert-True $m.Success 'не нашёл сбор изменённых файлов в гейте объёма'
    Assert-Match $m.Value '--grep=\^Task: \$Task' 'прошлые коммиты ветки не отбираются по трейлеру Task:'
    Assert-Match $m.Value 'git diff-tree --no-commit-id --name-only -r' 'файлы прошлых коммитов не собираются'
