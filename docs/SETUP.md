...
вида `password`/`api_key`/`token` с непустым длинным значением. Совпадение — коммит
блокируется, хук называет файл и номер строки, само значение не печатается. Правило на
присваивания — уже, чем в `detect-secrets`: там ключевое слово принимает суффикс
(`password_secure` тоже считается находкой), здесь ключевое слово обязано стоять сразу
