...
# Смотрелка процессов — Win32_Process, он есть только на Windows. Без этой проверки
# Unix-исключение глоталось тихо, список выходил пустым, и аварийный тормоз печатал
# «снимать нечего» при живых агентах — успех там, где сделано ровно ничего.
if ($IsLinux -or $IsMacOS) {
    Write-Host ''
    Write-BcfFail 'на Linux/macOS снятие процессов пока не реализовано: смотреть процессы нечем'
    Write-BcfNote 'останови прогон руками (kill воркеров) или с Windows-машины; мягкий путь работает:'
    Write-BcfNote 'bcf stop  — остановка между задачами через .bcf/STOP'
    Write-Host ''
    exit 2
}
