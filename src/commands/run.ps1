...
for ($i = 0; $i -lt $script:BcfArgs.Count; $i++) {
    ...
    switch -Regex ($a) {
        ...
        # Значение флага съедается тут же ($i++), иначе оно уходит в общую ветку как
        # позиционный аргумент: `--max-iterations 8` давало loop.ps1 лишний параметр «8»
        # и отказ «positional parameter cannot be found» (2026-09-09).
        '^--budget$'         { $budget = Get-BcfFlagNumber -Flag '--budget' -Index ($i + 1) -T ([double]); $i++; break }
        '^--concurrency$'    { $conc = Get-BcfFlagNumber -Flag '--concurrency' -Index ($i + 1) -T ([int]); $i++; break }
        '^--resume$'         { $resume = Get-BcfFlagValue -Flag '--resume' -Index ($i + 1); $i++; break }
        '^--max-iterations$' { $maxIter = Get-BcfFlagNumber -Flag '--max-iterations' -Index ($i + 1) -T ([int]); $i++; break }
