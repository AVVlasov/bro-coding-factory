...
$route = Invoke-Node -Role planner -Label 'роутер-риск' -Schema $ROUTE_SCHEMA -Prompt @"
...
$LENSES_USED = if ($heavy) { $LENSES } else { @($LENSES | Where-Object { $_.key -in @('память', 'гейты') }) }
...
Изменённые файлы: $(Get-GraphVar files)
Посмотреть правки: $(Get-GraphVar diffCmd) <файл>

Уже найдено другими (НЕ повторяй, ищи новое):
$(Get-GraphVar known)

Читай сам код, а не только диф: дефект часто в том, как правка сочетается с окружающим.
