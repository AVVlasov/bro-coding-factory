...
if ($failed) {
    ...
    # Спрашиваем КОНТЕКСТ ПРОГОНА, а не параметр скрипта: скрипт графа подключается
    # точкой, то есть делит с нами область видимости, и полагаться на то, что параметр
    # доживёт до конца в неизменном виде, нельзя. $ctx — то же, чем пользуется рантайм.
