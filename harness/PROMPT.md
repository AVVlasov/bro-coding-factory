...
### 2. Уроки памяти уже в STATE.md

Векторную память опрашивает раннер до итерации и кладёт находки блоком `<anti-patterns>`
в `.bcf/STATE.md`. Прочитай их и **не повторяй past failures**. Скрипты памяти руками не
зови: `memory/history/ask.py` в проектах нет, вызов уходит в ошибку и съедает шаг
(живой случай 2026-09-09: локальная модель звала его каждую итерацию). Паттерн «hobbling
