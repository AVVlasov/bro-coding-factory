...
a. `loop/STATE.md` — итог прошлой итерации, backpressure-ошибки.
...
| Фаза | Кто делает | Что |
|---|---|---|
| **0 — Planner** | ты | high-level decomposition в `loop/state/plan.json` (если ещё нет). Без технических деталей; каждый пункт — один-два слайса. |
| **A.5 — Contract Negotiation** ⭐ | ты + harness | Перед кодом — напиши `loop/state/contract.json`: granular DoD-ассерты с проверяемыми критериями. Договорённость с судьёй ДО кода. Без contract'а судья даст NEEDS-MORE-EVIDENCE. |
...
- `loop/state/plan.json` (Phase 0)
- `loop/state/contract.json` (Phase A.5)
- `loop/state/STATE.json` (derived автоматически из events.jsonl — не пиши его сам!)

> ⚠️ **Пути — ВСЕГДА repo-relative.** Пиши `loop/state/plan.json`, а НЕ абсолютный путь.
...
**НЕ запускай тестеров и судью сам.** Создай маркер `loop/VERIFY-REQUEST` (формат ниже).
...
- **events.jsonl** — append-only журнал. Harness сам пишет события; ты НЕ пишешь туда.
  Inspect: `pwsh loop/inspect.ps1 --task <TASK-ID>`.
- **lint-gate** — backpressure-проверка (`loop/lint-gate.ps1`). Падения приходят в
...
- **Запускать harness-скрипты руками:** `loop/lint-gate.ps1`, `loop/verify.ps1`,
  `loop/triggers.ps1`, `loop/loop.ps1`, `loop/inspect.ps1`. Их крутит раннер между
  итерациями; результаты приходят через `loop/STATE.md` (BACKPRESSURE). Самозапуск =
  итерация будет убита по таймауту. Для верификации создавай `loop/VERIFY-REQUEST`.
- **Polling-loop по файлам harness'а** (`while (-not (Test-Path loop/VERIFY-REQUEST)) {…}`,
...
- **Слепая вера в стейл-вердикт.** Если вердикт ссылается на несуществующую сейчас
  ...
  прошлого. Не «лечи» это кодом; перепиши `loop/VERIFY-REQUEST` и переверифицируй.
